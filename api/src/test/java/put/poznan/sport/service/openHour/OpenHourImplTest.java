...
class OpenHourImplTest {
    ...
    @Test
    void updateOpenHour_ShouldUpdateAndReturnOpenHour() {
        // given
        int openHourId = 1;
        OpenHour existingOpenHour = OpenHour.builder()
                .id(openHourId)
                .monday(new OpeningTime(LocalTime.of(8, 0), LocalTime.of(20, 0)))
                .build();

        OpenHour updatedOpenHour = OpenHour.builder()
                .id(openHourId)
                .monday(new OpeningTime(LocalTime.of(9, 0), LocalTime.of(21, 0)))
                .build();

        when(openHourRepository.findById(openHourId)).thenReturn(Optional.of(existingOpenHour));
        when(openHourRepository.save(any(OpenHour.class))).thenReturn(updatedOpenHour);

        // when
        OpenHour result = openHourService.updateOpenHour(updatedOpenHour);

        // then
        assertNotNull(result);
        assertEquals(openHourId, result.getId());
        assertEquals(LocalTime.of(9, 0), result.getMonday().getStart());
        assertEquals(LocalTime.of(21, 0), result.getMonday().getEnd());
        verify(openHourRepository).save(updatedOpenHour);
    }

    @Test
    void updateOpenHour_ShouldThrowException_WhenNotFound() {
        // given
        OpenHour openHour = OpenHour.builder()
                .id(999)
                .monday(new OpeningTime(LocalTime.of(8, 0), LocalTime.of(20, 0)))
        ...
        when(openHourRepository.findById(999)).thenReturn(Optional.empty());

        // when & then
        assertThrows(OpenHourNotFoundException.class, 
            () -> openHourService.updateOpenHour(openHour));
    }

    @Test
    void deleteOpenHourById_ShouldDeleteOpenHour_WhenExists() {
        // given
        int openHourId = 1;
        OpenHour openHour = OpenHour.builder()
                .id(openHourId)
        ...
        // when
        openHourService.deleteOpenHourById(openHourId);
    ...
    @Test
    void deleteOpenHourById_ShouldThrowException_WhenNotFound() {
        // given
        int openHourId = 999;
        when(openHourRepository.findById(openHourId)).thenReturn(Optional.empty());
