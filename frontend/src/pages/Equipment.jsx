...
import "/src/index.css"; // Ensure this is the correct path for Tailwind CSS
...
const Equipment = () => {
    ...
    useEffect(() => {
        const fetchEquipment = async () => {
            try {
                ...
                // Pobranie obrazów dla każdego sprzętu
    ...
    const handleDelete = async (equipmentId) => {
        try {
            ...
            setEquipment(equipment.filter(item => item.id !== equipmentId)); // Remove equipment from the list after deletion
        } catch (error) {
        ...
        <div className="container mx-auto p-4">
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {equipment.map((item) => (
                        <div key={item.id} className="bg-white shadow-md rounded-lg overflow-hidden p-4 flex flex-col items-center">
                            {/* Wyświetlanie obrazu lub placeholdera */}
